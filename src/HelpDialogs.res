...
module TieBreaks = {
  ...
  let make = (~state, ~ariaLabel) =>
    <BaseDialog state ariaLabel>
      ...
      <dl>
        <dt className="title-20"> {s(Buchholz)} </dt>
        <dd>
          {`Sum all of each player's opponents' scores.`->React.string}
        </dd>
        <dt className="title-20"> {s(BuchholzCut1)} </dt>
        <dd>
          {`Sum all of each player's opponents' scores, discarding the lowest.`->React.string}
        </dd>
        <dt className="title-20"> {s(SonnebornBerger)} </dt>
        <dd>
          {`Sum the scores of opponents that a player has defeated, plus half the scores of opponents
            with whom the player has drawn.`->React.string}
        </dd>
